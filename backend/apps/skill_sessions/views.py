...
from core.permissions import IsSessionParticipant
...
class SendSessionRequestView(APIView):
    ...
    def post(self, request):
        ...
        session = send_session_request(sender = request.user, validated_data=serializer.validated_data)
...
class AcceptSessionView(APIView):
    ...
    def patch(self, request, session_id):
        ...
        if not result:
            return error_response(
                message='You cannot accept this session',
                status_code=400
            )
        return success_response(
...
class DeclineSessionView(APIView):
    ...
    def patch(self, request, session_id):
        ...
        if not result:
            return error_response(
                message='You cannot decline this session',
                status_code=400
            )
        return success_response(
            data=SessionRequestSerializer(result).data,
...
class ComleteSessionView(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def patch(self, request, session_id):
        ...
        if not result:
            return error_response(
                message="You cannot complete this session",
                status_code=400
            )
        return success_response(
            data=SessionRequestSerializer(result).data,
...
class SubmitReviewView(APIView):
    ...
    def post(self, request, session_id):
        ...
        if not review:
            return error_response(
                message="You cannot review this session",
                status_code=400
            )
...
class CreditHistoryView(APIView):
    ...
    def get(self, request):
        ...
        return success_response(
            data=serializer.data,
            message='Credit history fected succesfully'
        )