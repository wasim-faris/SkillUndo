...
def add_skill_to_user(user, skill_id, skill_type):
    ...
    user_skill = UserSkill.objects.create(user=user, skill=skill, skill_type=skill_type)
    return user_skill
...
def remove_skill_from_user(user, skill_id):
    ...
    """
    skill = UserSkill.objects.filter(user=user, id=skill_id).delete()
...
def get_matches_for_user(user):
    ...
    user_want = UserSkill.objects.filter(user=user, skill_type="learn").values_list(
        "skill_id", flat=True
    )

    # What current user teaches
    user_teaches = UserSkill.objects.filter(user=user, skill_type="teach").values_list(
        "skill_id", flat=True
    )
