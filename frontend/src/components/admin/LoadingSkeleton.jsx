    ...
    return (
      <div className={`grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-5 ${className}`}>
        {Array.from({ length: rows }).map((_, index) => (
          <div
            key={index}
            className="animate-pulse rounded-2xl border border-[var(--admin-border)] bg-[var(--admin-card)] p-4 shadow-[0_16px_40px_rgba(0,0,0,0.18)] sm:p-5"
          >
          ...
          <thead className="bg-[var(--admin-surface)]">
            <tr>
              {Array.from({ length: columns }).map((_, index) => (
                <th key={index} className="px-4 py-4 text-left sm:px-5">
