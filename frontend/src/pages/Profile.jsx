...
const CardHeader = memo(function CardHeader({ icon, title, action }) {
  return (
...
const Dots = memo(function Dots({ max = 5, filled = 3 }) {
  return (
...
const ProfileSkillsSection = memo(function ProfileSkillsSection({ type, skills, loading, onAdd, canAdd = true }) {
        ...
        <div className="mx-auto max-w-[900px] space-y-5 px-4 sm:px-0">
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="card-premium overflow-hidden">
            ...
            <div className="px-6 pb-6">
              ...
              <div className="relative z-10 mb-4 -mt-14 flex flex-col items-start gap-4 sm:flex-row sm:items-end sm:justify-between">
                ...
                {isOwnProfile ? (
                  <button onClick={() => openEditProfile()} className="mb-2 flex w-full items-center justify-center gap-1.5 rounded-xl border border-[var(--border-default)] bg-[var(--bg-secondary)] px-4 py-2 text-[13px] font-medium text-[var(--text-primary)] transition-all hover:border-[var(--accent-primary)] hover:text-[var(--accent-primary)] sm:w-auto">
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
            <Card className="group">
              <CardHeader icon="📝" title="About" action={isOwnProfile ? <EditBtn onClick={() => openEditProfile()} /> : null} />
                ...
                {activityLoading ? (
                  ...
                  <div className="relative space-y-0">
                    ...
                    {activity.map((item, i) => {
                      ...
                      const diffMs = createdAt ? Date.now() - createdAt.getTime() : null; // eslint-disable-line react-hooks/purity
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
            <Card>
              <CardHeader icon="📅" title="Availability" action={isOwnProfile ? <EditBtn onClick={() => openEditProfile()} /> : null} />
