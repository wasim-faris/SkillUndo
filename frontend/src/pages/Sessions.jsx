...
function SessionDetailModal({ sessionId, onClose, reviewed, onReviewSubmitted }) {
  ...
  const handleJoinMeeting = async () => {
    if (!session?.meeting_link || joining) return;
    setJoining(true);
    try {
...
function SessionCard({ session, currentUserId, pendingAction, onAction, onOpenDetail, reviewed }) {
  ...
  return (
    ...
    >
      ...
      {session.status === 'confirmed' && (
        <div className={`mb-3 flex items-center justify-between gap-2 rounded-xl border px-3 py-2 ${session.meeting_link ? 'bg-[rgba(52,211,153,0.06)] border-[rgba(52,211,153,0.25)]' : 'bg-[var(--bg-secondary)] border-[var(--border-default)]'}`}>
          <div className="flex items-center gap-2 min-w-0">
            {session.meeting_link ? (
              <div className="flex items-center gap-1.5 min-w-0 text-xs font-bold text-[var(--accent-green)]">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 shrink-0">
                  <path d="M4.5 4.5a3 3 0 00-3 3v9a3 3 0 003 3h8.25a3 3 0 003-3v-9a3 3 0 00-3-3H4.5zM19.94 18.75l-2.69-2.69V7.94l2.69-2.69c.944-.945 2.56-.276 2.56 1.06v11.38c0 1.336-1.616 2.005-2.56 1.06z" />
                </svg>
                <span className="truncate">Meeting Room Ready</span>
              </div>
            ) : (
              <div className="flex items-center gap-1.5 min-w-0 text-[10px] font-bold text-[var(--text-muted)]">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 shrink-0">
                  <path fillRule="evenodd" d="M12 1.5a5.25 5.25 0 00-5.25 5.25v3a3 3 0 00-3 3v6.75a3 3 0 003 3h10.5a3 3 0 003-3v-6.75a3 3 0 00-3-3v-3c0-2.9-2.35-5.25-5.25-5.25zm3.75 8.25v-3a3.75 3.75 0 10-7.5 0v3h7.5z" clipRule="evenodd" />
                </svg>
                <div className="flex flex-col">
                  <span className="truncate">Meeting Locked</span>
                  <span className="text-[9px] font-normal leading-tight">Available 10 mins before</span>
                </div>
              </div>
            )}
          </div>
          {session.meeting_link ? (
            <button
              onClick={handleJoinMeeting}
              disabled={joining}
              className="shrink-0 flex items-center gap-1 px-3 py-1 rounded-lg text-[10px] font-bold text-white transition-all hover:scale-[1.03] active:scale-[0.97] shadow-sm disabled:opacity-70 disabled:hover:scale-100 disabled:cursor-not-allowed"
              style={{ background: 'linear-gradient(135deg, #34d399 0%, #059669 100%)' }}
            >
            ...
            <button
              disabled
              className="shrink-0 flex items-center gap-1 px-3 py-1.5 rounded-lg text-[10px] font-bold text-[var(--text-muted)] bg-[var(--bg-card)] border border-[var(--border-default)] cursor-not-allowed opacity-60"
            >
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-3 h-3">
                  <path fillRule="evenodd" d="M12 1.5a5.25 5.25 0 00-5.25 5.25v3a3 3 0 00-3 3v6.75a3 3 0 003 3h10.5a3 3 0 003-3v-6.75a3 3 0 00-3-3v-3c0-2.9-2.35-5.25-5.25-5.25zm3.75 8.25v-3a3.75 3.75 0 10-7.5 0v3h7.5z" clipRule="evenodd" />
              </svg>
...
export default function Sessions() {
  ...
  const handleAction = async (action, session) => {
    ...
    if (optimisticStatuses[action]) {
      // Spread ALL existing session fields so nothing (e.g. meeting_link) is discarded
    ...
    try {
      ...
      if (action === 'complete') {
        ...
        // The list endpoint may not include meeting_link, so we hydrate it here.
        try {
          const detailRes = await getSessionDetail(session.id);
          const fullSession = unwrap(detailRes);
          if (fullSession) {
            applySessionUpdate(session.id, () => fullSession);
          }
        } catch {
